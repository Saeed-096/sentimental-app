  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:4000/analyze-chat/",
        formData
      );
      if (response.error) {
          ...
          <>
            ...
            <div className="relative mb-4 sm:mb-6 md:mb-8 ">
              <p className="text-white/80 mb-2 sm:mb-3 md:mb-4 text-sm sm:text-base">
                Upload a txt file containing chat
              </p>
          ...
          {chats.length > 0 && (
            <div className="bg-[#3C0A1D] rounded-lg overflow-auto max-h-64">
              <table className="w-full text-white">
                <thead>
                  <tr>
                    <th className="text-left p-4 font-bold">CHAT</th>
                ...
                <tbody>
                  {chats.map((chat, index) => (
                    <tr key={index} className="border-t border-white/10">
                      <td className="p-4 text-white/90">{chat.Message}</td>
                      <td className="p-4 text-white/90">
                        {chat.Sentiment || "N/A"}
                      </td>
                      <td className="p-4">
                        <div className="w-10 h-10">
                          <SentimentEmoji sentiment={chat.Sentiment} />
                        </div>
                      </td>
                    </tr>
                  ))}
